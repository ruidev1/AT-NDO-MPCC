...
% 
% MPC_vars.qC = 1;
% MPC_vars.qVtheta= 0.5;
% MPC_vars.qL= 1500;
% 
% times = 2;
% run_simulation;
% PlotInfo(eC_log, v_log, simN, 0.02, 'qC=2');
% 
% % eC_ratio = (exp(-abs(eC_log(2, :))) - exp(-eC_max)) ./ (1 - exp(-eC_max));
% % reward_step(2, :) = slope * eC_ratio + intercept;
% reward(2) = sum(reward_step(times,:));
% % PlotInfo(eC_log, v_log, simN, 0.02, 'qVtheta=0.002');
% 
% initialization;
% 
...
% times = 3;
% run_simulation;
% PlotInfo(eC_log, v_log, simN, 0.02, 'qC=200');
% % eC_ratio = (exp(-abs(eC_log(2, :))) - exp(-eC_max)) ./ (1 - exp(-eC_max));
% % reward_step(3, :) = slope * eC_ratio + intercept;
% reward(3) = sum(reward_step(times,:));
% % PlotInfo(eC_log, v_log, simN, 0.02, 'qVtheta=0.002');
% %% Tuning Weight RUn
% simN = 50;
% initialization;
% MPC_vars.qC = 0.1;
% for j = 1: 25
%     if get_curvature(traj, x0(ModelParams.stateindex_theta)) > 2
%         MPC_vars.qC = 1;
%     else
%         MPC_vars.qC = 0.1;
%     end
%     run_simulation;
%     PlotParameters(MPC_vars, simN, 1, j);
% end
% PlotInfo(eC_log, v_log, 500, 0.02, 'qC=adaptive');
