...
function [X,U,dU,info] = getMPCmatrices(traj,MPC_vars,ModelParams,borders,Xhor,Uhor,x0,u0, dhat)
...
for i = 1:MPC_vars.N
    ...
    % linearized dynamics
    [stage(i).Ak,stage(i).Bk,stage(i).gk] = getEqualityConstraints(Xk,Uk,MPC_vars,ModelParams, dhat);
...
function [Ak,Bk,gk] = getEqualityConstraints(Xk,Uk,MPC_vars,ModelParams, dhat) 

    nx = ModelParams.nx;
    nu = ModelParams.nu;
    % linearize and discretize nonlinear bicycle model
    [Ad, Bd, gd]=DiscretizedLinearizedModel(Xk,Uk,ModelParams,MPC_vars.Ts, dhat);
